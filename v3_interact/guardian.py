...
# v1_loop 最后插（最高优先），确保 `config` 找到 v1_loop/config.py
...
from chat_window import open_chat
from context_builder import build_first_message

# v2_notify/notifier.py 与本目录同名，用 importlib 按路径加载，避免循环引用
...
def trigger_help(history: list, result: dict):
    """弹出通知，用户选择后打开对话窗"""
    reason = result.get("reason", "检测到异常屏幕状态")
    suggestions = result.get("suggestions", [])

    def on_select(idx: int, text: str):
...
def run():
    log.info("=" * 55)
    log.info("Screen Guardian v3 启动  Ctrl+C 退出")
